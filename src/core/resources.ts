...
import {getSupportedNetworks, getRpcUrl, DEFAULT_NETWORK, type ChainId} from "./chains.js";
...
export function registerEVMResources(server: McpServer) {
  ...
  // Default chain info (Base mainnet)
  server.resource(
    "default_chain_info",
    "evm://chain",
    async (uri) => {
      try {
        const network = DEFAULT_NETWORK; // 'base' by default
        const chainId = await services.getChainId(network);
        const blockNumber = await services.getBlockNumber(network);
        const rpcUrl = getRpcUrl(network);
  ...
  // Default latest block (Base mainnet)
  server.resource(
          ...
          contents: [{
            ...
            text: JSON.stringify({
              ...
              balance: {
                ...
                eth: balance.eth // use 'eth' instead of 'ether'
              }
            }, null, 2)
          }]
        };
  ...
  // Default native token balance (Base mainnet)
  server.resource(
    "default_native_balance",
    new ResourceTemplate("evm://address/{address}/balance", { list: undefined }),
    async (uri, params) => {
      try {
        const network = DEFAULT_NETWORK; // 'base' by default
        const address = params.address as string;
  ...
  // Default ERC20 balance (Base mainnet)
  server.resource(
  ...
  // Default transaction by hash (Base mainnet)
  server.resource(
  ...
  // Circle CCTP Resources
  
  // Get CCTP supported chains
  server.resource(
    "cctp_supported_chains",
    "cctp://chains",
    async (uri) => {
      try {
        const supportedChains = await import('./services/cctp.js').then(m => m.getSupportedCCTPChains());
  ...
  // Get USDC balance for CCTP chains
  server.resource(
    "cctp_usdc_balance",
    new ResourceTemplate("cctp://{chainId}/usdc/balance/{address}", { list: undefined }),
    async (uri, params) => {
      try {
        const chainId = parseInt(params.chainId as string);
        const address = params.address as string;
        const { getUSDCBalance } = await import('./services/cctp.js');
        const balance = await getUSDCBalance(address as Address, chainId as ChainId);
